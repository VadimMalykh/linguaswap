...
- **All development and testing must happen inside Docker.** Do not run `mix`, `mix test`, or any Elixir/Ecto commands locally. Use `docker compose exec app <command>` for all operations (e.g., `docker compose exec app mix test`, `docker compose exec app mix precommit`). The only exception is editor tooling (e.g., ElixirLS) which may run locally against the mounted source.
...
- **Always** be mindful of where to place routes. `phx.gen.auth` creates multiple router plugs:
  ...
  - A plug `redirect_if_user_is_authenticated` that redirects to a default path in case the user is authenticated - useful for a registration page that should only be shown to unauthenticated email
