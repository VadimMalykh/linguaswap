defmodule LinguaswapWeb.UserAuthTest do
  use LinguaswapWeb.ConnCase, async: true

  alias Linguaswap.Accounts
  alias Linguaswap.Accounts.Scope
  alias LinguaswapWeb.UserAuth

  import Linguaswap.AccountsFixtures

  @remember_me_cookie "_linguaswap_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LinguaswapWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "require_sudo_mode/2" do
    test "allows email that have authenticated in the last 10 minutes", %{conn: conn, user: user} do
      conn =
        conn
        |> fetch_flash()
        |> assign(:current_scope, Scope.for_user(user))
